...
/* Random-OVS: Calculate weight equal */
static int *
random_mod_weight(int paths, int *origin_weight){
    // sum of weights
    int i;
    int weightsum, equalweight = 0;
    for(i = 0; i < paths; i++)
        weightsum+=origin_weight[i];
    equalweight = weightsum/paths;

    // array that stores sorted weight and index
    int ** sorted_weight = (int**)malloc(sizeof(int*) * paths);
    
    for(i = 0; i < paths; i++)
        sorted_weight[i] = (int*)malloc(sizeof(int) * 2);

    for(i = 0; i < paths; i++) {
        sorted_weight[i][0] = origin_weight[i];
        sorted_weight[i][1] = i;
    }

    int * real_weights = (int*)malloc(sizeof(int) * paths);
    for(i = 0; i < paths; i++)
        real_weights[sorted_weight[i][1]] = equalweight;
    return real_weights;
}

/* VALO-OVS: Recalculate bucket weights */
static int *
mod_weight(int paths, int *origin_weight)
{
    int i, j;
    // array that stores sorted weight and index
    int ** sorted_weight = (int**)malloc(sizeof(int*) * paths);
    ...
    int curr, curr_idx;
    for(i = 1; i < paths; i++) {
        curr = sorted_weight[i][0];
        curr_idx = sorted_weight[i][1];
        for(j = i-1; j >= 0 && sorted_weight[j][0] < curr ; j--) {
            sorted_weight[j+1][0] = sorted_weight[j][0];
            sorted_weight[j+1][1] = sorted_weight[j][1];
        }

        sorted_weight[j+1][0] = curr;
        sorted_weight[j+1][1] = curr_idx;
    }

    // set arrays for matrix multiplication
    int ** A = (int**)malloc(sizeof(int*) * paths);
    int * A_mul_r = (int*)malloc(sizeof(int) * paths);
    int ** B = (int**)malloc(sizeof(int*) * paths);
    int * B_mul_A_mul_r = (int*)malloc(sizeof(int) * paths);
    for(i = 0; i < paths; i++) {
        A[i] = (int*)malloc(sizeof(int) * paths);
        B[i] = (int*)malloc(sizeof(int) * paths);
    }
    ...
    float * calc_weights = (float*)malloc(sizeof(float) * paths);
    calc_weights[0] = 1;
    ...
    for(i = 1; i < paths; i++) {
        calc = 1;
        for(j = 0; j < i; j++)
            calc *= pow((float)B_mul_A_mul_r[j+1]/(float)B_mul_A_mul_r[j], 1.0/(float)(j+1));

        calc_weights[i] = calc;
    }

    for(i = 0; i < paths; i++)
        sorted_weight[i][0] = (int) (calc_weights[i] * 1000.0);

    int * real_weights = (int*)malloc(sizeof(int) * paths);
    for(i = 0; i < paths; i++)
        real_weights[sorted_weight[i][1]] = sorted_weight[i][0];

    // free all arrays
    for(i = 0; i < paths; i++)
    {
        free(sorted_weight[i]);
        free(A[i]);
        free(B[i]);
    }
    free(sorted_weight);
...
table_fitting(int paths,int *origin_weight){
    int tableSizeT;
    tableSizeT = 10;
    int * reducedWeights = (int *)malloc(sizeof(uint16_t) * paths);
    int sumOfreducedWeights = 0;
    int sumOfweights = 0;
    for (int i=0;i<paths;i++){
        reducedWeights[i] = origin_weight[i];
        sumOfreducedWeights += origin_weight[i];
    ...
    while (sumOfreducedWeights > tableSizeT){
        ...
        for (int i=0;i<paths;i++){
            if (reducedWeights[i] == 1){
                nonReducibleSize += reducedWeights[i]; 
            }
        }
        if (nonReducibleSize == paths){
            break;
        }
        double reductionRatio = ((double)tableSizeT-nonReducibleSize)/(double)sumOfweights;
        for (int i=0;i<paths;i++){
            reducedWeights[i] = (int) round((double) origin_weight[i] * reductionRatio);
            if (reducedWeights[i] == 0){
                reducedWeights[i] = reductionRatio;
            }
            newSumOfreducedWeights += reducedWeights[i];
        }
                               ...
                               const struct ofputil_group_mod *gm)
{
    ...
    /* VALO-OVS: Count paths, recalculate bucket weights if valo is enabled */
    int paths = 0;
    LIST_FOR_EACH (bucket, list_node, &gm->buckets) {
        paths++;
    }

    int * bucket_weight = (int*)malloc(sizeof(int) * paths);
    paths = 0;
    LIST_FOR_EACH (bucket, list_node, &gm->buckets) {
